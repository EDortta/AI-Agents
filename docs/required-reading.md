...
| `~/.config/email/send.py` | opcional | o transporte de e-mail **do AI-Agents** — declarado aqui porque `.docs/workflows/sending-email.md` exige que cada projeto declare o seu; não vale para nenhum outro repositório |
| `~/.config/email/credentials.conf` | opcional | credenciais do transporte acima; nunca citar |

### Lista de destinatários

**Não existe.** O AI-Agents não tem destinatário recorrente nem regra de CC. Todo envio
daqui é pontual: pergunte ao operador quem recebe. `.docs/workflows/sending-email.md`
proíbe deduzir isso do e-mail que o harness reporta, que identifica a conta logada e não
o operador.
